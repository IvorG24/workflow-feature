...
import CreateSupplier from "../QuotationFormPage/SupplierList/CreateSupplier";
import SupplierList from "../QuotationFormPage/SupplierList/SupplierList";
import FormDetailsSection from "../RequestFormPage/FormDetailsSection";
import FormSection from "../RequestFormPage/FormSection";
import CreateService from "./ServiceList/CreateService";
...
const SubconFormPage = ({
  ...
  const [selectedService, setSelectedService] =
    useState<ServiceWithScopeType | null>(null);
  const [serviceList, setServiceList] = useState(services);
  const [serviceCount, setServiceCount] = useState(serviceListCount);
  ...
  const handleFormVisibilityRestriction = async () => {
    try {
      const result = await checkSubconFormStatus(supabaseClient, {
        teamId: team.team_id,
        formId: `${formId}`,
      });
      return result;
      ...
      {!switchValue ? (
        <Box>
          ...
          <Paper p="xl" shadow="xs">
            {!selectedService ? (
              <Center>
                <Text color="dimmed">No service selected</Text>
              </Center>
            ) : null}
            {selectedService ? (
              <ServiceScope
      ...
      {switchValue ? (
        <Stack spacing="xl">
          ...
          <FormSection
            section={{
              ...form.form_section[1],
              section_field: form.form_section[1].section_field.slice(0, 1),
            }}
          />
        </Stack>
      ) : null}
