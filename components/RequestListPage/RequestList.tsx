...
function RequestList({ requestList }: RequestListProps) {
      ...
      <Flex direction="column" align="center">
        {requestList.map((request) => (
          <Box
            ...
            onClick={() => router.push(`/teams/${router.query.teamName}/requests/${request.request_id}`)}
            styles={{
              "&:hover": {
                cursor: "pointer",
          ...
          >
            <Text size="lg" weight="bold" m={0}>
              {startCase(request.form_name as string)}
            </Text>
            <Text size="lg" weight="bold" m={0}>
