import { OptionType, TechnicalInterviewFilterFormValues } from "@/utils/types";
import {
  Accordion,
  Button,
  Drawer,
...
type Props = {
  fetchData: (data?: TechnicalInterviewFilterFormValues) => void;
  handleReset: () => void;
      ...
      >
        ...
        >
          <Stack spacing="xs" sx={{ overflow: "hidden" }}>
            <Accordion variant="contained" defaultValue="Technical Interview">
              ...
              <Accordion.Item value="Technical Interview">
                <Accordion.Control> {label} Interview</Accordion.Control>
                <Accordion.Panel>
                  <Stack spacing="xs" sx={{ overflow: "hidden" }}>
                    <Stack spacing={0}>
                      <Text size={14} fw={500}>
                        {label} Interview Date Created
                      </Text>
                      <Flex gap="xs">
                        <Controller
                          control={control}
                          name="technical_interview_date_created.start"
                          render={({ field: { value, onChange } }) => {
                            const newValue = value
                        ...
                        <Controller
                          control={control}
                          name="technical_interview_date_created.end"
                          render={({ field: { value, onChange } }) => {
                            const newValue = value
                              ? new Date(value as string)
                              : null;
                    ...
                    <Controller
                      control={control}
                      name="technical_interview_status"
                      render={({ field: { value, onChange } }) => {
                        const newValue = value ?? [];
                        return (
                          <MultiSelect
                    ...
                    <Stack spacing={0}>
                      ...
                      <Flex gap="xs">
                        <Controller
                          control={control}
                          name="technical_interview_schedule.start"
                          render={({ field: { value, onChange } }) => {
                            const newValue = value
                              ? new Date(value as string)
                              : null;
                        ...
                        <Controller
                          control={control}
                          name="technical_interview_schedule.end"
                          render={({ field: { value, onChange } }) => {
                            const newValue = value
                              ? new Date(value as string)
                              : null;
...
export default TechnicalInterviewFilterMenu;
