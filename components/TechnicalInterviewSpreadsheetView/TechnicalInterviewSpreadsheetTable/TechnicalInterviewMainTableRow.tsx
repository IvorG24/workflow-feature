...
import { updateTechnicalInterviewSchedule } from "@/backend/api/update";
...
const TechnicalInterviewMainTableRow = ({
  ...
  const handleSchedule = async (schedule: string) => {
    try {
      ...
      await updateTechnicalInterviewSchedule(supabaseClient, {
  ...
  return (
    <tr className={classes.row}>
      ...
      >
        <Stack spacing="xl">
          <Text color="dimmed" size={14}>
            Technical Interview schedule for{" "}
      ...
      <td>
        {item.technical_interview_status === "WAITING FOR SCHEDULE" && (
