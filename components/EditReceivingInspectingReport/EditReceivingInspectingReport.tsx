...
const EditReceivingInspectingReportPage = ({
  request,
  itemOptions,
  originalItemOptions,
  requestProjectId,
  ...
  const handleDuplicateSection = (sectionId: string) => {
    if (
      ...
      formSections.length === originalItemOptions.length + 2
    ) {
      notifications.show({
  ...
  const handleItemChange = async (
    ...
    const newOption = originalItemOptions.find(
      (option) =>
        parseJSONIfValid(option.option_value) ===
        parseJSONIfValid(prevValue || "")
    );
    if (newOption) {
      setAvailableItems((prev) => {
...
export default EditReceivingInspectingReportPage;
